...
main() {
  ...
  readarray -t valid_timezones <<<"$(cat "$SCRIPT_DIR/configs/valid_timezones.txt")"
  ...
  [ "$CONFIGURE_LETSENCRYPT" == true ] || [ "$ASSUME_SSL" == true ] && bash "$SCRIPT_DIR/lib/verify-fqdn.sh" "$FQDN"
